(async () => {
    ...
    const requestedAt = document.getElementById('requested-at');
    // The server provides UTC; display it in the phone's local time zone.
    try {
        requestedAt.textContent = new Intl.DateTimeFormat(
            document.documentElement.lang || undefined, {
                year: 'numeric', month: '2-digit', day: '2-digit',
                hour: '2-digit', minute: '2-digit', second: '2-digit',
                timeZoneName: 'short',
            }).format(new Date(requestedAt.dateTime));
    } catch (error) {
        // Keep the explicit UTC timestamp if local formatting is unavailable.
    }
